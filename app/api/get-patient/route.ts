...
export async function GET(request: Request) {
	...
	try {
		if (!email || !doctorId)
			throw new Error('Patient email and doctor ID are required');
		data =
			await sql`SELECT name, email, image, phone_number, status FROM patients WHERE email = ${email}`;
		patients = data.rows;
		if (patients.length > 0) {
			...
			if (responseData.verificationResult === 'TRUE') {
				...
				return NextResponse.json('patient not in your location', {
					status: 200
				});
			}
		}
